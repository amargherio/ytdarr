...
A destination conflict does not overwrite an existing file. Correct the destination or source issue rather than forcing an overwrite. Changed/missing sources, unavailable permissions, and blocked or in-progress states can create recovery records. Use **Retry recovery** or **Retry cleanup** after fixing the cause; do not delete recovery records to make them disappear.
